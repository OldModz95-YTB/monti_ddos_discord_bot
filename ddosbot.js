...
Client.on('ready', ()=>{
    ...
    Client.user.setActivity('Cracked By ProtonDev.', { type: 'WATCHING'}).catch(console.error);
})
...
/*Client.on("message", message => {
    ...
    if(message.content.startsWith(prefix + "haste")) {
        ...
        hastebin(haste).then(r => {
            ...
            console.error()
            //message.edit(":x: An error has occurred. Details: " + console.error)
            );
    }
...
Client.on("message",async message => {
 ...
 Client.on('guildMemberAdd', member =>{
    ...
    channel.send(` Welcome to ProtonDev, ${member}. Other than that, enjoy your stay you filthy nonce.`)
});
...
Client.on('message', (message)=>{
    ...
    if(message.content.startsWith(prefix + "ddos")) {
      ...
      if(message.content.startsWith(prefix + "911")) {
        if (message.deletable) message.delete();
    ...
    if(command === "info") {
           ...
           .addBlankField()
           .setAuthor("Bot by OldModz95| 悲しみはあなたの親友です。")
   ...
   var roasts = roast[Math.floor(Math.random() * roast.length)];
    ...
    if(command === "commands") {
        ...
        .addField('**$OldModz95**', ' [ OldModz95 luvs u] ')
        .setColor('#40FF00')
        .setFooter('nulled.to')
        .setThumbnail ("https://www.searchpng.com/wp-content/uploads/2018/12/Cartoon-Bomb-Clipart-715x715.png")
      ...
      if(command === "tools") {
        ...
        .addField('**$OldModz95**', ' [ OldModz95 luvs u] ')
        .setColor('#080707')
        .setFooter('Bot coded by ProtonDev. [OldModz95]')
      ...
      if(command ==="OldModz95") {
      const m = message.channel.send("OldModz95! 😳");
      let embed = new Discord.MessageEmbed()
    .setTitle("𝔐𝔬𝔫𝔱𝔦")
    .setAuthor(" OldModz95 is a russian teenager living within the slumps of Bomb Squad. You will find him hanging around with Mom, Murphy and Kui smoking a joint talking about cute girls.")
    .setColor('#080707')
